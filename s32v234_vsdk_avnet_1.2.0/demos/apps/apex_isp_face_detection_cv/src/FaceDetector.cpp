...
void FaceDetector::GetFaces(void* lpFrame)
{
  ...
  Resample((char*)pyramidStartFrame.GetDataPtr(), (char*)lpFrame, pyramidStartSize.width, pyramidStartSize.height, 1280,
           720);
  GrayScale((unsigned char*)dataInImages[0].GetDataPtr(), (unsigned char*)pyramidStartFrame.GetDataPtr(),
            pyramidStartFrame.GetWidth(), pyramidStartFrame.GetHeight());
  ResampleG((char*)dataInImages[1].GetDataPtr(), (char*)dataInImages[0].GetDataPtr(),
            (sizes[0].width * CHUNK_WIDTH_1) / CHUNK_WIDTH_0, (sizes[0].height * CHUNK_WIDTH_1) / CHUNK_WIDTH_0,
            sizes[0].width, sizes[0].height);
  for(int i = 1; i < SIZES; ++i)
  {
    int originalImageIndex = i % 2;
    ResampleG((char*)dataInImages[i].GetDataPtr(), (char*)dataInImages[originalImageIndex].GetDataPtr(), sizes[i].width,
              sizes[i].height, sizes[originalImageIndex].width, sizes[originalImageIndex].height);
  }
...
#define FACE_DET_GR_OUT_INTEGR_IMG "OUTPUT_INTEGRAL_IMAGE"
  ...
  sizes[0].Set((pyramidStartSize.width * CHUNK_WIDTH_1) / CHUNK_WIDTH_0,
               (pyramidStartSize.height * CHUNK_WIDTH_1) / CHUNK_WIDTH_0);
...
DetectedFace::DetectedFace(FaceRectangle& faceRectangle)
    : testingFaceRectangle(faceRectangle), minFaceRectangle(faceRectangle), maxFaceRectangle(faceRectangle),
...
bool DetectedFace::TryAddFaceRectangle(FaceRectangle& faceRectangle)
{
  ...
  if(faceRectangle.x < minFaceRectangle.x)
    minFaceRectangle.x = faceRectangle.x;
  if(faceRectangle.y < minFaceRectangle.y)
    minFaceRectangle.y = faceRectangle.y;
  if(faceRectangle.width < minFaceRectangle.width)
    minFaceRectangle.width = faceRectangle.width;
  if(faceRectangle.height < minFaceRectangle.height)
    minFaceRectangle.height = faceRectangle.height;

  if(faceRectangle.x > maxFaceRectangle.x)
    maxFaceRectangle.x = faceRectangle.x;
  if(faceRectangle.y > maxFaceRectangle.y)
    maxFaceRectangle.y = faceRectangle.y;
  if(faceRectangle.width > maxFaceRectangle.width)
    maxFaceRectangle.width = faceRectangle.width;
  if(faceRectangle.height > maxFaceRectangle.height)
    maxFaceRectangle.height = faceRectangle.height;
...
{
  return FaceRectangle((minFaceRectangle.x + maxFaceRectangle.x) >> 1, (minFaceRectangle.y + maxFaceRectangle.y) >> 1,
