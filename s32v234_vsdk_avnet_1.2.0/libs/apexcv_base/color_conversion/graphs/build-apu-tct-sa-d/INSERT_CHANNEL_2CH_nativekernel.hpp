#ifndef _ACF_PROCESS_APU_INSERT_CHANNEL_2CH_NATIVE_KERNEL_WRAPPER
#define _ACF_PROCESS_APU_INSERT_CHANNEL_2CH_NATIVE_KERNEL_WRAPPER
...
where:
   ...
   [2] corresponds to input 'CH_ID'
   [3] corresponds to output 'OUTPUT_0'

and 'pArgs = (void*)MYKERNEL_args;'
******************************************************/

static void INSERT_CHANNEL_2CH_nativekernel(void* pArgs)
{
   acf_io_info* lpIo = 0;
   ...
   CH_ID.Init2D(0, (void*)lpIo->pBuf, lpIo->width, lpIo->height, lpIo->span, lpIo->elementDataType, lpIo->elementDimX, lpIo->elementDimY);
   ...
   INSERT_CHANNEL_2CH lProcess;
   lProcess.Initialize();
   lProcess.ConnectIO("INPUT_0", INPUT_0);
   lProcess.ConnectIO("INPUT_1", INPUT_1);
   lProcess.ConnectIO("CH_ID", CH_ID);
   lProcess.ConnectIO("OUTPUT_0", OUTPUT_0);
   lProcess.Start();
   lProcess.Wait();
}

#endif //_ACF_PROCESS_APU_INSERT_CHANNEL_2CH_NATIVE_KERNEL_WRAPPER
