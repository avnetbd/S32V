#ifndef _ACF_PROCESS_APU_ACCUMULATE_WEIGHTED_08U_NATIVE_KERNEL_WRAPPER
#define _ACF_PROCESS_APU_ACCUMULATE_WEIGHTED_08U_NATIVE_KERNEL_WRAPPER
...
where:
   ...
   [2] corresponds to input 'ALPHA'
   [3] corresponds to output 'OUTPUT_0'

and 'pArgs = (void*)MYKERNEL_args;'
******************************************************/

static void ACCUMULATE_WEIGHTED_08U_nativekernel(void* pArgs)
{
   acf_io_info* lpIo = 0;
   ...
   ALPHA.Init2D(0, (void*)lpIo->pBuf, lpIo->width, lpIo->height, lpIo->span, lpIo->elementDataType, lpIo->elementDimX, lpIo->elementDimY);
   ...
   ACCUMULATE_WEIGHTED_08U lProcess;
   lProcess.Initialize();
   lProcess.ConnectIO("INPUT_0", INPUT_0);
   lProcess.ConnectIO("INPUT_1", INPUT_1);
   lProcess.ConnectIO("ALPHA", ALPHA);
   lProcess.ConnectIO("OUTPUT_0", OUTPUT_0);
   lProcess.Start();
   lProcess.Wait();
}

#endif //_ACF_PROCESS_APU_ACCUMULATE_WEIGHTED_08U_NATIVE_KERNEL_WRAPPER
