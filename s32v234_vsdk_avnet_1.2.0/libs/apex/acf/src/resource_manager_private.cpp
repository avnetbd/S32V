...
static const bool gscAreTracesEnabled = false;
         ...
         else
         {
               ...
               areValuesInInputArrayValid == true)
            {
               ...
               *(cpReturnedSizeInBytes) = neededSizeInBytes;
               ...
               {
                  //
                  // SASHBY: memset and placement new could be trouble if combined with complex types
                  // in the "ResourceManager" struct which allocate resources in their constructors!!!!
                  //
                  memset(cpThis, 0, sizeof(*cpThis));

                  new ((void*)cpThis) ResourceManager; // Placement new
                                          
                  libResult = GDC_OAL_SemaphoreCreate(&(cpThis->mThreadSafetySemaphore),
                                                      "NXP_ResMgr",
                                                      GDC_OAL_SEMAPHORE_COUNTING,
                                                      1); /* "count" should not be used in mutex semaphore */
                  if(libResult == LIB_SUCCESS)
                  {
                     libResult = GDC_OAL_SemaphoreCreate(&(cpThis->mUsers.mOalSemaphore),
                                                         "NXP_ResMgr_Users",
                                                         GDC_OAL_SEMAPHORE_COUNTING,
                     ...
                     if(libResult == LIB_SUCCESS)
                     {
                        ...
                        cpThis->mpBufferNext += cpThis->mScratchPadSizeInBytes;
                           ...
                           for(i = 0; i < cpThis->mUsers.mNumberOfUsers; ++i, --j)
                           {
                              ...
                              cpThis->mpBufferNext                = (cpThis->mpBufferNext + (sizeof(uintptr_t) - 1)) & ~(sizeof(uintptr_t) - 1); /* Start aligned */
                        cpUser->mpInputArray                = (uint32_t (*)[4])(cpThis->mpBufferNext);
                     ...
                     libResult = ResMgr_User_Type_Register_Private(cpThis,
                                                                         pcResMgr,
                                                                         resMgrNameLength,
                                                                         userOutputArray,
                                                                         cNumberOfUserNames);
                        ...
                        libResult = ResMgr_User_Allocate_Private(0x9,
                                                                       cpThis,
...
LIB_RESULT ResMgr_Deinitialize_Private(struct ResourceManager* const cpThis)
{
   ...
   if(cpThis != 0 &&
      cpThis->mIsInitialized == true)
   {
      libResult = GDC_OAL_SemaphoreObtain(cpThis->mThreadSafetySemaphore, 0xFFFFFFFF);

      if(libResult == LIB_SUCCESS)
      {
         cpThis->mIsInitialized = false;

         libResult = GDC_OAL_SemaphoreDelete(cpThis->mThreadSafetySemaphore);
         ...
         if(libResult == LIB_SUCCESS)
         {
            ...
            for(i = 0; i < cpThis->mUsers.mNumberOfUsers; ++i)
            {
               ...
               {
                  libResult = GDC_OAL_SemaphoreDelete(cpThis->mUsers.mpUsers[i].mOalSemaphore);
                                       ...
                                       uint32_t const cSizeOfArrays)
{
      ...
      cSizeOfArrays > 1) /* At least one entry for the set type and one resource type */
   {
      ...
      if(libResult == LIB_SUCCESS)
      {
                     ...
                     if(cpThis->mpBufferNext <= cpThis->mpBufferEnd)
                     {
                        for(j = 0; j < cpSetType->mNumberOfResourceTypes; ++j)
                        {
                           ...
                           if(cpThis->mpBufferNext <= cpThis->mpBufferEnd)
                           {
                              ...
                              {
                                 cpSet->mppResources[j][k].mOwnerUserId   = cpThis->mResMgr_UserId;
                                 cpSet->mppResources[j][k].mWaitingUserId = cpThis->mResMgr_UserId;
         ...
         libIntResult = GDC_OAL_SemaphoreRelease(cpThis->mThreadSafetySemaphore); SHOW_ERROR(libIntResult == LIB_SUCCESS);
      }
   }
 ...
 *    cpReturnedUserId
 */
LIB_RESULT ResMgr_User_Allocate_Private(const uint32_t,
                                        ...
                                        uint32_t* const cpReturnedUserId)
{
      ...
      cpReturnedUserId != 0)
   {
      ...
      libResult = GDC_OAL_SemaphoreObtain(cpThis->mUsers.mOalSemaphore, 0xFFFFFFFF); /* Blocks if there are no available users objects */
            ...
            if(cUserTypeId_Index < cpThis->mUsers.mNumberOfUserTypes)
            {
               if(cpThis->mUsers.mNumberOfFreeIndexes > 0)
               {
                  ...
                  *(cpReturnedUserId) = userIndex + (((uint32_t)pUser->mSessionId) << 16);  /* Storing session id in the high 16 bits of user id */
                  ...
                  libResult = GDC_OAL_SemaphoreCreate(&(pUser->mOalSemaphore),
                                                  "FSL_ResMgr_Usr",
                                                  GDC_OAL_SEMAPHORE_COUNTING,
                                                  0);
               }
               ...
               {
                  libIntResult = GDC_OAL_SemaphoreRelease(cpThis->mUsers.mOalSemaphore); SHOW_ERROR(libIntResult == LIB_SUCCESS);
 ...
 */
LIB_RESULT ResMgr_User_Deallocate_Private(const uint32_t,
                                          struct ResourceManager* const cpThis,
      ...
      cpThis->mIsInitialized == true)
   {
      ...
      if(libResult == LIB_SUCCESS)
      {
            ...
            cpThis->mUsers.mNumberOfFreeIndexes < cpThis->mInits.mUser.mMaxNumberOfObject)
         {
            ...
            libResult = GDC_OAL_SemaphoreDelete(cpUser->mOalSemaphore);  /* SASHBY: Maybe keep semaphores around to not incur costs for reallocating same users */
         }
      ...
      if(weFreedUser == true)
      {
         libResult = GDC_OAL_SemaphoreRelease(cpThis->mUsers.mOalSemaphore);
      }
   }
 ...
 */
static LIB_RESULT ResMgr_Helper_Output_Array_Validate(struct ResourceManager* const cpThis,
 ...
 */
#if 0
static LIB_RESULT ResMgr_Helper_Resource_Allocate_TT(struct ResourceManager* const cpThis,
                                                     uint32_t const cUserId,
         ...
         if(areAllResourcesAvailable == true) /* Go ahead and allocate them */
         {
            ...
            for(i = 0; i < cSizeOfInputArray; ++i)
            {
               ...
               for(j = 0; j < cInputArray[i][2]; ++j)
               {
                  ...
                  SHOW_ERROR(cpSet->mppResources[cInputArray[i][1]][outputArray[k][4]].mOwnerUserId == cpThis->mResMgr_UserId);

                  cpSet->mppResources[cInputArray[i][1]][outputArray[k][4]].mOwnerUserId = cUserId;
                  ++k;
               }
            }
            cpUser->mNumberOfAllocatedResources = totalNumberOfResources;

            SHOW_ERROR(LIB_SUCCESS == ResMgr_Helper_Output_Array_Validate(cpThis,
                                                                               outputArray,
                                                                               cSizeOfOutputArray));
 ...
 */
#if 0
static LIB_RESULT ResMgr_Helper_Resource_Allocate_TI(struct ResourceManager* const cpThis,
                                                     uint32_t const cUserId,
                                                     uint32_t cInputArray[][3],
                                                     uint32_t const cSizeOfInputArray,
                                                     uint32_t outputArray[][5],
         ...
         while(pSetType != 0)
         {
            ...
            for(i = 0; i < pSetType->mNumberOfSets; ++i)
            {
               ...
               for(j = setTypeBegin; j < cSizeOfInputArray; ++j)
               {
                  ...
                  if(pcSet->mppResources[pSortedInputArray[j][1]][pSortedInputArray[j][2]].mOwnerUserId != cpThis->mResMgr_UserTypeIndex) /* Means this instance is allocated/not free */
                  {
                     areAllResourcesAvailable = false;
                     break; /* This set instance is missing resources */
                  }
 ...
 *            when 4 is <Resource instance number>
 */
static LIB_RESULT ResMgr_Helper_Resource_Allocate_II(struct ResourceManager* const cpThis,
                                                     uint32_t const cUserId,
                                                     uint32_t cInputArray[][4],
                                                     uint32_t const cSizeOfInputArray,
                                                     uint32_t outputArray[][5],
                                                     uint32_t const cSizeOfOutputArray,
                                                     bool const cIsCalledFrom_ScanWaitUserQueue)
{
   LIB_RESULT libResult = LIB_FAILURE;
      ...
      cpThis->mScratchPadSizeInBytes >= (cSizeOfInputArray * sizeof(cInputArray[0])) + (gcResMgr_MaxNumberOfSetTypes * sizeof(cInputArray[0][0])))
   {
      ...
      uint32_t (*pSortedInputArray)[4] = (uint32_t(*)[4])cpThis->mpScratchPad;       /* *4 is the number of element in each entry of cInputArray */
      ...
      qsort (cpThis->mpScratchPad,
             cSizeOfInputArray,
             sizeof(cInputArray[0]),
             ResMgr_Helper_qsort_compare_uint32_t_x4_3_2_1_0);

      if(pSortedInputArray[cSizeOfInputArray-1][0] < cpThis->mSetTypes.mNumberOfSetTypes &&  /* Check validity of values */
         pSortedInputArray[cSizeOfInputArray-1][1] < cpThis->mSetTypes.mpSetTypes[pSortedInputArray[cSizeOfInputArray-1][0]].mNumberOfSets &&
         pSortedInputArray[cSizeOfInputArray-1][2] < cpThis->mSetTypes.mpSetTypes[pSortedInputArray[cSizeOfInputArray-1][0]].mNumberOfResourceTypes &&
         pSortedInputArray[cSizeOfInputArray-1][3] < cpThis->mSetTypes.mpSetTypes[pSortedInputArray[cSizeOfInputArray-1][0]].mpNumberOfResourcePerType[pSortedInputArray[cSizeOfInputArray-1][2]])
      {
         uint32_t i = 0;
         ...
         for(i = 0; i < (cSizeOfInputArray-1); ++i)
         {
            ...
            if(pSortedInputArray[i][0] >=  cpThis->mSetTypes.mNumberOfSetTypes ||  /* Check validity of values */
               pSortedInputArray[i][1] >=  cpThis->mSetTypes.mpSetTypes[pSortedInputArray[i][0]].mNumberOfSets ||
               pSortedInputArray[i][2] >=  cpThis->mSetTypes.mpSetTypes[pSortedInputArray[i][0]].mNumberOfResourceTypes ||
               pSortedInputArray[i][3] >=  cpThis->mSetTypes.mpSetTypes[pSortedInputArray[i][0]].mpNumberOfResourcePerType[pSortedInputArray[i][2]] ||
               (pSortedInputArray[i][0] == pSortedInputArray[i+1][0] &&
                pSortedInputArray[i][1] == pSortedInputArray[i+1][1] &&
                pSortedInputArray[i][2] == pSortedInputArray[i+1][2] &&
                pSortedInputArray[i][3] == pSortedInputArray[i+1][3])) /* Check for duplicates */
            {
               areInputValuesValid = false;
               break;
            }
      ...
      if(areInputValuesValid == true)
      {
         ...
         /* Check that all the resources requested by the caller are free */
         for(i = 0; i < cSizeOfInputArray; ++i)
         {
            const uint32_t cSetType     = pSortedInputArray[i][0];
            const uint32_t cSetInstance = pSortedInputArray[i][1];
            const uint32_t cResType     = pSortedInputArray[i][2];
            const uint32_t cResInstance = pSortedInputArray[i][3];

            struct ResMgr_Resource* const cpResource = &(cpThis->mSetTypes.mpSetTypes[cSetType].mpSets[cSetInstance].mppResources[cResType][cResInstance]);

            if(cpResource->mOwnerUserId    != cpThis->mResMgr_UserTypeIndex ||  /* The resource is not free */
               (cpResource->mWaitingUserId != cpThis->mResMgr_UserTypeIndex &&  /* Someone else is waiting on it */
                cpResource->mWaitingUserId != cUserId))
            {
               areAllResourcesAvailable = false;
            }

            if(cIsCalledFrom_ScanWaitUserQueue == true &&
               cpResource->mWaitingUserId == cpThis->mResMgr_UserTypeIndex)
            {
               cpResource->mWaitingUserId = cUserId;
            }
         }
         ...
         if(areAllResourcesAvailable == true) /* Go ahead and allocate them */
         {
            ...
            for(i = 0; i < cSizeOfInputArray; ++i)
            {
               struct ResMgr_Set* const cpSet = &(cpThis->mSetTypes.mpSetTypes[cInputArray[i][0]].mpSets[cInputArray[i][1]]);

               outputArray[k][0] = i;
               outputArray[k][1] = cInputArray[i][0]; // Set Type
               outputArray[k][2] = cInputArray[i][1]; // Set Instance
               outputArray[k][3] = cInputArray[i][2]; // Res Type
               outputArray[k][4] = cInputArray[i][3]; // Res Instance

               SHOW_ERROR(cpSet->mppResources[outputArray[k][3]][outputArray[k][4]].mOwnerUserId == cpThis->mResMgr_UserId);

               cpSet->mppResources[outputArray[k][3]][outputArray[k][4]].mOwnerUserId = cUserId;

               /* Need remove the instance from the free list and compact it */
               weStartCompacting = false;

               for(j = 0; j < (cpSet->mpNumberOfFreeResources[outputArray[k][3]] - 1); ++j)
               {
                  if(cpSet->mppFreeResourceIndexes[outputArray[k][3]][j] == outputArray[k][4])
                  {
                     weStartCompacting = true;
                  }
                  ...
                  if(weStartCompacting == true)
                  {
                     cpSet->mppFreeResourceIndexes[outputArray[k][3]][j] = cpSet->mppFreeResourceIndexes[outputArray[k][3]][j+1];
                  }
               }

               --(cpSet->mpNumberOfFreeResources[outputArray[k][3]]);

               cpSet->mppFreeResourceIndexes[outputArray[k][3]][cpSet->mpNumberOfFreeResources[outputArray[k][3]]] = ~(0U);
 ...
 * Hmm
 */
static LIB_RESULT ResMgr_Helper_ResetWaitingUserIds(const uint32_t,
                                                     struct ResourceManager* const cpThis)
{
   LIB_RESULT libResult = LIB_FAILURE;
   ...
   if(cpThis != 0)
   {
      const uint32_t cNumberOfSetTypes        = cpThis->mSetTypes.mNumberOfSetTypes;
      struct ResMgr_SetType* const cpSetTypes = cpThis->mSetTypes.mpSetTypes;

      uint32_t i = 0; // SetTypes
      uint32_t j = 0; // SetInstances
      uint32_t k = 0; // ResourceTypes
      uint32_t l = 0; // ResourceInstances

      for(i = 0; i < cNumberOfSetTypes; ++i)
      {
         const uint32_t cNumberOfSets = cpSetTypes[i].mNumberOfSets;

         for(j = 0; j < cNumberOfSets; ++j)
         {
            const uint32_t cNumberOfResTypes = cpSetTypes[i].mNumberOfResourceTypes;

            for(k = 0; k < cNumberOfResTypes; ++k)
   {
               const uint32_t cNumberOfRes = cpSetTypes[i].mpNumberOfResourcePerType[k];

               for(l = 0; l < cNumberOfRes; l++)
      {
                  cpSetTypes[i].mpSets[j].mppResources[k][l].mWaitingUserId = cpThis->mResMgr_UserId;
               }
            }
         }
      }
 ...
 * Hmm
 */
static LIB_RESULT ResMgr_Helper_ScanWaitingUserQueue(const uint32_t cThreadId,
                                                     struct ResourceManager* const cpThis)
{
   LIB_RESULT libResult = LIB_FAILURE;

   if(cpThis != 0 &&
      cpThis->mWaitingUserQueue.size() > 0)
         {
      libResult = ResMgr_Helper_ResetWaitingUserIds(cThreadId,
                                                    cpThis); SHOW_ERROR(libResult == LIB_SUCCESS);

      std::deque<uint32_t>::iterator iteratorCurrent = cpThis->mWaitingUserQueue.begin();
      std::deque<uint32_t>::iterator iteratorEnd     = cpThis->mWaitingUserQueue.end();

      for(;iteratorCurrent != iteratorEnd; ++iteratorCurrent)
      {
         const uint32_t cQueuedUserIndex = (*iteratorCurrent) & 0xFFFF; /* SASHBY: Magic Number Bad!!!
                                                                         * Lower 16 bytes is user index, high 16 bytes is user session id
                                                                         */
            struct ResMgr_User* const cpQueuedUser = &(cpThis->mUsers.mpUsers[cQueuedUserIndex]);

         if(cpQueuedUser->mIsAllocatingSetInstances       == true &&
            cpQueuedUser->mIsAllocatingResourcesInstances == true)
            {
            libResult = ResMgr_Helper_Resource_Allocate_II(cpThis,
                                                           (*iteratorCurrent),
                                                              cpQueuedUser->mpInputArray,
                                                              cpQueuedUser->mSizeOfInputArray,
                                                              cpQueuedUser->mpOutputArray,
                                                           cpQueuedUser->mSizeOfOutputArray,
                                                           true); // cIsCalledFrom_ScanWaitUserQueue
         }
         else if(cpQueuedUser->mIsAllocatingSetInstances       == false &&
                 cpQueuedUser->mIsAllocatingResourcesInstances == true)
         {
            SHOW_ERROR(false); // Functionality disabled for the moment
#if 0
            libResult = ResMgr_Helper_Resource_Allocate_TI(cpThis,
                                                           (*iteratorCurrent),
                                                           (uint32_t (*)[3])(cpQueuedUser->mpInputArray),
                                                           cpQueuedUser->mSizeOfInputArray,
                                                           cpQueuedUser->mpOutputArray,
                                                              cpQueuedUser->mSizeOfOutputArray);
#endif
                 ...
                 cpQueuedUser->mIsAllocatingResourcesInstances == false)
            {
            SHOW_ERROR(false); // Functionality disabled for the moment
#if 0
            libResult = ResMgr_Helper_Resource_Allocate_TT(cpThis,
                                                           (*iteratorCurrent),
                                                           (uint32_t (*)[3])(cpQueuedUser->mpInputArray),
                                                              cpQueuedUser->mSizeOfInputArray,
                                                              cpQueuedUser->mpOutputArray,
            ...
            if(libResult == LIB_SUCCESS)
            {
            cpThis->mWaitingUserQueue.erase(iteratorCurrent);
            cpThis->mWaitingUserQueueSize = cpThis->mWaitingUserQueue.size();

            if(gscAreTracesEnabled == true) { printf("            %d %6X UNBLOCKED\n", cThreadId, (*iteratorCurrent)); }

            libResult = GDC_OAL_SemaphoreRelease(cpQueuedUser->mOalSemaphore);
         ...
         {
            if(gscAreTracesEnabled == true) { printf("            %d %6X STILL WAITING\n", cThreadId, (*iteratorCurrent)); }
            }
         }
                                               ...
                                               uint32_t const cSizeOfOutputArray)
{
      ...
      cSizeOfOutputArray >= cSizeOfInputArray)
   {
      ...
      if(libResult == LIB_SUCCESS)
      {
         libResult = ResMgr_Helper_ScanWaitingUserQueue(cThreadId,
                                                        cpThis);
         if(libResult == LIB_SUCCESS)
         {
            libResult = ResMgr_Helper_Resource_Allocate_TT(cpThis,
            ...
            if(libResult == LIB_RESOURCES_NOT_CURRENTLY_AVAILABLE)
            {
               ...
               cpUser->mIsAllocatingSetInstances       = false;
               cpUser->mIsAllocatingResourcesInstances = false;
               cpUser->mIsWaitingOnResources  = true;

               cpThis->mWaitingUserQueue.push_back(cUserId);
               cpThis->mWaitingUserQueueSize = cpThis->mWaitingUserQueue.size();
            ...
            {
               if(gscAreTracesEnabled == true) { printf("            %d %6X ALLOC\n", cThreadId, cUserId); }
            }
         }
      ...
      {
         if(gscAreTracesEnabled == true) { printf("            %d %6X WAIT\n", cThreadId, cUserId); }

         libResult = GDC_OAL_SemaphoreObtain(cpUser->mOalSemaphore, 0xFFFFFFFF);
         ...
         if(libResult == LIB_SUCCESS)
         {
            ...
            if(libResult == LIB_SUCCESS)
            {
               if(gscAreTracesEnabled == true) { printf("            %d %6X OBTAINED\n", cThreadId, cUserId); }
      ...
      if(libResult == LIB_SUCCESS)
      {
         ...
         if(libResult == LIB_SUCCESS)
         {
            ...
            if(libResult == LIB_RESOURCES_NOT_CURRENTLY_AVAILABLE)
            {
               ...
               cpUser->mIsAllocatingSetInstances       = false;
               cpUser->mIsAllocatingResourcesInstances = true;
               cpUser->mIsWaitingOnResources  = true;
                                               ...
                                               uint32_t const cSizeOfOutputArray)
{
      ...
      cSizeOfOutputArray >= cSizeOfInputArray)
   {
      ...
      libResult = GDC_OAL_SemaphoreObtain(cpThis->mThreadSafetySemaphore, 0xFFFFFFFF);
               ...
               {
         libResult = ResMgr_Helper_ScanWaitingUserQueue(cThreadId,
                                                        cpThis);     /* SASHBY: What is the point of scanning before an allocate
                                                                      *         I can understand doing it after a deallocate...
                                                                      */
         if(libResult == LIB_SUCCESS)
         {
            libResult = ResMgr_Helper_Resource_Allocate_II(cpThis,
                                                           ...
                                                           outputArray,
                                                           cSizeOfOutputArray,
                                                           false); // cIsCalledFrom_ScanWaitUserQueue
            ...
            if(libResult == LIB_RESOURCES_NOT_CURRENTLY_AVAILABLE)
            {
               ...
               cpUser->mIsAllocatingSetInstances       = true;
               cpUser->mIsAllocatingResourcesInstances = true;
               cpUser->mIsWaitingOnResources           = true;
      ...
      if(libResult == LIB_SUCCESS)
      {
            ...
            cpUser->mIsFree == false)
         {
            ...
            for(i = 0; i < cSizeOfInputArray; ++i)
            {
               ...
               if(cpSet->mppResources[inputArray[j][3]][inputArray[j][4]].mOwnerUserId == cUserId)
               {
                  cpSet->mppResources[inputArray[j][3]][inputArray[j][4]].mOwnerUserId = cpThis->mResMgr_UserId;
            ...
            if(libResult == LIB_SUCCESS)
            {
               if(gscAreTracesEnabled == true) { printf("            %d %6X FREE\n", cThreadId, cUserId); }
